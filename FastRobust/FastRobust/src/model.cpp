...
bool Model::onInit(const std::string& strFilename)
{
  ...
  if( idx != std::string::npos)
	  m_path.append(strFilename.substr(0, idx + 1));
...
{  
   ...
   for(unsigned int boneId=0;boneId<vectorCoreBone.size();++boneId)
   {
...
void Model::setAnimation(int id, float delay){
	if(id < m_animationCount){
		for(int i=0; i<m_animationCount; i++){
			if(i==id)
				m_calModel->getMixer()->blendCycle(i, 1.0f, delay);
			else
				m_calModel->getMixer()->clearCycle(i, delay);
		}
	}
}
