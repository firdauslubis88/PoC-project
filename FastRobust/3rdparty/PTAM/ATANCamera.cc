...
Matrix<4> ATANCamera::MakeUFBLinearFrustumMatrix(double near, double far)
{
  ...
  /*
  // Computation in camera coordinates (on the 'near' plane in the frustum)
  ...
  */
  ...
  // The same computation in image coordinates (on the image plane)
  // Above code can be replaced with following lines
  // Refer to ARToolKit code ('argConvGLcpara2' function in GL/gsub.c)
  m4[0][0] = (2 * mvFocal[0]) / (mvImageSize[0]);
  m4[1][1] = -(2 * mvFocal[1]) / (mvImageSize[1]);
  m4[0][2] = (2 * mvCenter[0]) / (mvImageSize[0]) - 1.0;
  m4[1][2] = (2 * mvCenter[1]) / (mvImageSize[1]) - 1.0;
...
// From ARToolKit Default Parameters (640x480)
//const Vector<NUMTRACKERCAMPARAMETERS> ATANCamera::mvDefaultParams = makeVector(
//	1.095236672, 1.512696212, 0.4953125, 0.504166667, 0.1);
// PTAM Calibration (640x480)
const Vector<NUMTRACKERCAMPARAMETERS> ATANCamera::mvDefaultParams = makeVector(
	0.919645, 1.22906, 0.50667, 0.521586, 0 );
// PTAM Calibration (800x480)
//const Vector<NUMTRACKERCAMPARAMETERS> ATANCamera::mvDefaultParams = makeVector(
//	0.915849, 1.52968, 0.509088, 0.667816, 0 );