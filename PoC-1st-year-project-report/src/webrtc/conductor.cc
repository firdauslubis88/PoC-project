...
Conductor::Conductor(PeerConnectionClient* client, ofApp_webrtc* main_wnd)
  : peer_id_(-1),
    loopback_(false),
    client_(client),
    main_wnd_(main_wnd) {
...
bool Conductor::InitializePeerConnection() {
  ...
  if (!peer_connection_factory_.get()) {
    ...
    main_wnd_->MessageBox("Error",
		...
		*/
	  std::cout << "Failed to initialize PeerConnectionFactory" << endl;
    DeletePeerConnection();
    return false;
  }
  ...
  if (!CreatePeerConnection(DTLS_ON)) {
    ...
    main_wnd_->MessageBox("Error",
		...
		*/
	  std::cout << "CreatePeerConnection failed" << endl;
	  DeletePeerConnection();
  }
  AddStreams();
  return peer_connection_.get() != NULL;
...
//      LOG(WARNING) << "Failed to apply the received candidate";
	  std::cout << "Failed to apply the received candidate" << endl;
    }
    return;
  }
...
void Conductor::OnServerConnectionFailure() {
    ...
    main_wnd_->MessageBox("Error", ("Failed to connect to " + server_).c_str(),
                          true);
						  */
	std::cout << "Failed to connect to " + server_ << endl;
}
...
void Conductor::ConnectToPeer(int peer_id) {
  ...
  if (peer_connection_.get()) {
    ...
    main_wnd_->MessageBox("Error",
		...
		*/
	  std::cout << "We only support connecting to one peer at a time" << endl;
    return;
  }
...
//    main_wnd_->MessageBox("Error", "Failed to initialize PeerConnection", true);
	  std::cout << "Failed to initialize PeerConnection" << endl;
  }
}
